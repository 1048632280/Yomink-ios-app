...
final class CollectionReaderViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIGestureRecognizerDelegate {
    ...
    private func showHomeIndicatorDiagnosticsAlert() {
        ...
        lines.append("模型: \(UIDevice.current.model)")
        lines.append("界面方向: \(UIDevice.current.orientation.rawValue)")
        lines.append("")
        ...
        lines.append("view.window: \(view.window != nil ? "exists" : "NIL")")
        lines.append("isViewLoaded: \(isViewLoaded)")
        lines.append("")

        // ============ 4. Window / Scene ============
        lines.append("===== Window / Scene =====")
        let windowScene = view.window?.windowScene
        lines.append("windowScene: \(windowScene.map { String(describing: type(of: $0)) } ?? "nil")")
        if let ws = windowScene {
            lines.append("ws.activationState: \(ws.activationState.rawValue)")
            lines.append("ws.windows.count: \(ws.windows.count)")
            for (i, w) in ws.windows.enumerated() {
                lines.append("  win[\(i)]: isKey=\(w.isKeyWindow) hidden=\(w.isHidden) windowLevel=\(w.windowLevel.rawValue)")
                lines.append("    rootVC: \(w.rootViewController.map { String(describing: type(of: $0)) } ?? "nil")")
            }
        }
        lines.append("")
        ...
        var depth = 1
        while let vc = currentVC, depth <= 12 {
            let typeName = String(describing: type(of: vc))
            let shortName = typeName.count > 80 ? String(typeName.prefix(80)) + "…" : typeName
            lines.append("[\(depth)] \(shortName)")
            ...
            lines.append("    presentedVC: \(vc.presentedViewController.map { String(describing: type(of: $0)) } ?? "nil")")
            lines.append("    isModalInPresentation: \(vc.isModalInPresentation)")
            lines.append("    modalPresentationStyle: \(vc.modalPresentationStyle.rawValue)")
            currentVC = vc.parent
            depth += 1
        }
        lines.append("")
        ...
        if let nav = navigationController {
            lines.append("nav类型: \(type(of: nav))")
            lines.append("nav.topVC: \(nav.topViewController.map { String(describing: type(of: $0)) } ?? "nil")")
            lines.append("nav.viewControllers.count: \(nav.viewControllers.count)")
            lines.append("nav.prefersAutoHidden: \(nav.prefersHomeIndicatorAutoHidden)")
            lines.append("nav.deferEdges: \(nav.preferredScreenEdgesDeferringSystemGestures.rawValue)")
            lines.append("nav.childForAutoHidden: \(nav.childForHomeIndicatorAutoHidden.map { String(describing: type(of: $0)) } ?? "nil")")
            lines.append("nav.childForDefer: \(nav.childForScreenEdgesDeferringSystemGestures.map { String(describing: type(of: $0)) } ?? "nil")")
        } else {
            lines.append("nav: nil")
        }
        lines.append("")
        ...
        if let rootVC = view.window?.rootViewController {
            ...
            lines.append("rootVC类型: \(type(of: rootVC))")
            lines.append("rootVC.prefersAutoHidden: \(rootVC.prefersHomeIndicatorAutoHidden)")
            lines.append("rootVC.deferEdges: \(rootVC.preferredScreenEdgesDeferringSystemGestures.rawValue)")
            lines.append("rootVC.childForAutoHidden: \(rootVC.childForHomeIndicatorAutoHidden.map { String(describing: type(of: $0)) } ?? "nil")")
            lines.append("rootVC.childForDefer: \(rootVC.childForScreenEdgesDeferringSystemGestures.map { String(describing: type(of: $0)) } ?? "nil")")
            lines.append("")

            // 沿 childForHomeIndicatorAutoHidden 找链尾
            lines.append("===== childForAutoHidden 链 =====")
            var v: UIViewController = rootVC
            var step = 0
            while let next = v.childForHomeIndicatorAutoHidden, step < 10 {
                step += 1
                lines.append("[\(step)] \(String(describing: type(of: next)))")
                v = next
            }
            lines.append("链尾: \(String(describing: type(of: v)))")
            lines.append("链尾.prefersAutoHidden: \(v.prefersHomeIndicatorAutoHidden)")
            lines.append("链尾===self? \(v === self)")
            lines.append("")

            // 沿 childForScreenEdgesDeferringSystemGestures 找链尾
            lines.append("===== childForDefer 链 =====")
            v = rootVC
            step = 0
            while let next = v.childForScreenEdgesDeferringSystemGestures, step < 10 {
                step += 1
                lines.append("[\(step)] \(String(describing: type(of: next)))")
                v = next
            }
            lines.append("链尾: \(String(describing: type(of: v)))")
            lines.append("链尾.deferEdges: \(v.preferredScreenEdgesDeferringSystemGestures.rawValue)")
            lines.append("链尾===self? \(v === self)")
            lines.append("")
        }

        // ============ 8. Swizzle 验证 ============
        lines.append("===== Swizzle 验证 =====")
        if let rootVC = view.window?.rootViewController {
            let rootCls: AnyClass = type(of: rootVC)
            lines.append("rootVC 类: \(rootCls)")

            // 检查 rootVC 类的方法表里是否有我们注入的方法
            let selectors: [Selector] = [
                #selector(getter: UIViewController.childForHomeIndicatorAutoHidden),
            ...
            for sel in selectors {
                ...
                if let method = class_getInstanceMethod(rootCls, sel) {
                    let imp = method_getImplementation(method)
                    // 找出 IMP 所属的类(沿继承链)
                    var owner: AnyClass? = rootCls
                    var foundIn = "未知"
                    while let c = owner {
                        var count: UInt32 = 0
                        ...
                        if foundIn != "未知" { break }
                        owner = class_getSuperclass(c)
                    }
                    lines.append("  \(selName)")
                    lines.append("    IMP@\(String(format: "%p", UInt(bitPattern: unsafeBitCast(imp, to: Int.self))))")
                    lines.append("    实现在: \(foundIn)")
                } else {
                    lines.append("  \(selName): 无方法!")
                }
            }
        }
        lines.append("")

        // ============ 9. 主动调用 root 的 getter 看实际返回 ============
        lines.append("===== 主动求值 rootVC =====")
        if let rootVC = view.window?.rootViewController {
            lines.append("rootVC.prefersAutoHidden 返回: \(rootVC.prefersHomeIndicatorAutoHidden)")
            lines.append("rootVC.deferEdges 返回: \(rootVC.preferredScreenEdgesDeferringSystemGestures.rawValue)")
            lines.append("(deferEdges=4 表示 .bottom 生效)")
            lines.append("(deferEdges=0 表示 swizzle 没生效或链尾返回 [])")
        }
        lines.append("")
        ...
        view.window?.rootViewController?.setNeedsUpdateOfScreenEdgesDeferringSystemGestures()
        ...
        lines.append("已对 self / rootVC / nav 强制 setNeedsUpdate")
        ...
        alert.addAction(UIAlertAction(title: "复制全部", style: .default) { _ in
