...
struct ReaderTextSelectionPageContext {
    ...
    private let retainedFrame: CTFrame
    ...
    init?(
        ...
        pageBounds: CGRect,
        drawingContentRect: CGRect
    ) {
        guard attributedText.length > 0,
              pageBounds.width > 0,
              pageBounds.height > 0,
              drawingContentRect.width > 0,
              drawingContentRect.height > 0 else {
            return nil
        }

        let drawingRect = CGRect(
            x: drawingContentRect.minX,
            y: drawingContentRect.minY,
            width: max(1, drawingContentRect.width),
            height: max(1, drawingContentRect.height)
        )
        ...
        path.addRect(CGRect(origin: .zero, size: drawingRect.size))
    ...
    func rangeByMovingStart(of range: NSRange, to index: Int) -> NSRange {
        ...
        let start = min(max(proposed, 0), maximumStart)
    ...
    func selectionRects(for range: NSRange) -> [CGRect] {
        ...
        let lineRects = lines.compactMap { line -> CGRect? in
        ...
        return Self.selectionRectsConnectingLineGaps(lineRects)
    }
    ...
    private static func selectionRectsConnectingLineGaps(_ rects: [CGRect]) -> [CGRect] {
        ...
        for index in sorted.indices {
            ...
            guard index < sorted.index(before: sorted.endIndex) else {
                continue
            }
            ...
            let minX = min(current.minX, next.minX)
            let maxX = max(current.maxX, next.maxX)
            result.append(
                CGRect(
                    x: minX,
                    y: current.maxY - 0.5,
...
final class ReaderTextSelectionOverlayView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for rect in selectionRects where rect.intersects(bounds) {
            UIBezierPath(
    ...
    func clearSelection() {
        ...
        isHidden = true
        menuTargetView.resignFirstResponder()
        setNeedsDisplay()
    }
    ...
    func selectionOrHandleContains(_ point: CGPoint) -> Bool {
        selectionContains(point)
            || startHandle.frame.insetBy(dx: -16, dy: -16).contains(point)
            || endHandle.frame.insetBy(dx: -16, dy: -16).contains(point)
    }
...
private final class ReaderTextSelectionHandleView: UIControl {
    enum Kind {
        case start
        case end
    }
