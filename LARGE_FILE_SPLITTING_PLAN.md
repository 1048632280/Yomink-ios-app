...
| 2026-06-06 | 修复 Paging 拆分后的 widgetTimeFormatter 访问级别 | `CollectionReaderViewController.swift`、`LARGE_FILE_SPLITTING_PLAN.md` | 从构建日志确认 `widgetTimeFormatter` 因 `private` 跨文件不可见；已改为模块内 static let；待重新跑 CI/Xcode 编译确认 | 修复 `CollectionReaderViewController+Paging.swift` 调用 `widgetTimeFormatter` 的编译错误 |
| 2026-06-06 | 修复 Settings/Paging 拆分后的 layout helper 访问级别 | `CollectionReaderViewController+Settings.swift`、`CollectionReaderViewController+Paging.swift`、`LARGE_FILE_SPLITTING_PLAN.md` | 从构建日志确认 `applySettingsPanelPosition` 和 `rememberPagingLayoutSnapshot` 因 `private` 跨文件不可见；已改为模块内方法；待重新跑 CI/Xcode 编译确认 | 修复主文件 `viewDidLayoutSubviews` 调用拆分扩展 helper 的编译错误 |
| 2026-06-06 | 修复自动阅读启动锚点和分页边界抖动 | `CollectionReaderViewController.swift`、`CollectionReaderViewController+Paging.swift`、`LARGE_FILE_SPLITTING_PLAN.md` | `git diff --check` 通过；Windows 环境无 `xcodebuild`/`swift`，需在 Xcode 环境验证 | 自动阅读布局重开改用顶部锚点；垂直模式打开页时按请求 offset 对齐；分页 trim 改按真实可见顶部/底部留缓冲，避免短页仍在屏幕内时被裁剪 |
